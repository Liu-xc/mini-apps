...
- **ShaderMaterial 接 `#include <tonemapping_fragment>` 编译失败只进 console、画面黑天**
  → 回退手写 ACES+sRGB（math 与 three 一致）；`console.error` 已并入 `__game.errors`。
- **Toon 三阶量化会吞掉阴影对比** → gradientMap 阶距拉开（[60,152,255]）+ 压环境光。
- 自定义管线色彩：天空与雾必须同走 ACES+曝光1.2，地平线差一档就是一条亮带
  （像素取样对齐：带 (238,215,170) vs 天空 (207,196,167) → 低空暖抬升抹平）。

### 视觉评审轮 + 高清化（回应 Leo「美工质量/一点都不高清」，R1→R5）

- **R1 基线自审**：P0 零阴影/无分级/天空无层次/地面脏噪；P1 草花暗青/前景空/无描边/树泥。
- **落地**：PCFSoft 4K 阴影（太阳跟随玩家 ±38 局部高精度）、ACES Filmic（曝光1.2，
  天空手写同款曲线）、OutlineEffect 全场描边（sky/底板/草花/云显式关闭）、
  太阳光斑 additive 辉光、出生点营地（帐篷/篝火/栅栏/木柴/路牌/树桩 14 件 hero 道具）、
  远景林带剪影环、地表大尺度明暗斑块。
- **高清规格**：地面 1K→**2K**（anisotropy16）、HDRI 1K→**2K**、阴影 2K→**4K**、
  dpr=1 设备 **1.5× 超采样**（截图与普通屏边缘更锐）；4K 贴图因 18.5MB 仓库体积放弃改 2K。
- **素材扩容**：Kenney 子集 20→**52 款**（14 树/9 岩/4 灌木/6 草/7 花/12 营地道具）；
  草花由「硬提亮」改「贴图染色 `#6f9448` + 微 emissive」，修掉纸片白。
- **探针**：`__game.probe()` 实证 envReady/sunMapReady/scatterChildren=92/outline；
  营地道具 HEAD 全 200（初版「消失」是地形起伏遮挡+暗部藏匿，非加载失败）。
- **R5 终态**：阴影/描边/2K 地面/HDRI/营地/林带全部成立，亮带像素级消除，errs=[]。

### 遗留（进 M1）

- 草叶色调仍略偏青（已减淡，待与贴图统一调色）；天空蓝顶需抬头视角验证。
- 描边在远距离物体上偏弱（thickness 按屏幕空间，M1 分档）；云仅一层 Sprite。
- 低模卡通风已定型（ADR-002）：若要求照片级写实属换素材体系，需另拍板。
