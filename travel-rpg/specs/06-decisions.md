...
## ADR-006 渲染审美拉满路线与 AO 暂缓

- **状态**：已采纳（2026-09-24，it-012）
- **背景**：Leo 定调「审美拉满」。保风格化资产方向，渲染质感一档拉满。
- **决策**：
  - **水面反射**：water.ts 重构为 three Reflector 管线（512 RT + clipBias），
    自定义 shader 把倒影按菲涅尔/水深权重融入原水深混色/岸沫/波光，波纹法线
    扰动反射 UV。每帧额外一次 512 下采样场景渲染，成本可控。
  - **植被密度**：近圈 3200→4600（IAB 桌面帧率可承受范围）。
  - **AO 暂缓**：N8AO v2（全白）与 GTAOPass（半屏黑）均和自研 OutlineRenderPass
    （needsSwap=false + HalfFloat + OutlineEffect 直渲）冲突，needsSwap 双向复现。
    接地感暂由 4K 软阴影+顶点色暗角承担。复活前提：改标准 RenderPass 结构或
    换预烘焙暗角。
- **后果**：反射为屏幕空间平面反射，湖面外水域无效；AO 缺失的悬浮感部分由
  描边+阴影补偿；性能档位（移动端）仍为 M2 记录项。
