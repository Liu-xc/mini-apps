...
th { font-size: 8.5pt; text-align: left; color: #86909C; font-weight: 600;
  border-bottom: 0.45mm solid #C9CDD4; padding: 1.5mm 2mm; }
td { font-size: 8.6pt; line-height: 1.38; padding: 1.3mm 2mm; border-bottom: 0.3mm solid #E5E6EB;
  vertical-align: top; }
...
.appendix-shot img { width: 27mm; height: 60mm; object-fit: cover; object-position: top;
...
def build():
    ...
    def crow(c):
        return (f"<tr><td class='c'><b>{c[0]}</b></td><td>{esc(c[1])}</td><td class='c'>{esc(c[2])}</td>"
                f"<td class='c'><span class='tag {c[3].lower()}'>{c[3]}</span></td><td>{esc(c[4])}</td></tr>")
    half = (len(COMMONS) + 1) // 2
    crows_a = "".join(crow(c) for c in COMMONS[:half])
    crows_b = "".join(crow(c) for c in COMMONS[half:])
    thead = ('<tr><th style="width:9mm">#</th><th>问题</th><th style="width:30mm">涉及页面</th>'
...
<section class="page">
  ...
  <p class="lead">问题清单：P0×3 / P1×12 / P2×15。P0 与红线项均经 uiautomator dump、源码核对或 logcat 实锤（评审方法见下）。</p>
  <div class="statrow">{stats}</div>
  <div class="h2">评审方法</div>
  <div class="mrow">{methods}</div>
  <div class="h2">跨页面共性问题（优先修 · C1–C{half}）</div>
  <table>
    {thead}
    {crows_a}
  </table>
  <div class="pfoot"><span>总评</span><span style="margin-left:auto">{pageno}</span></div>
    ...
    pages.append(f"""
<section class="page">
  <div class="phead"><span class="no">00</span><span class="t">共性问题（续）</span></div>
  <p class="lead">共性问题 C{half + 1}–C{len(COMMONS)}；级别徽标：<span class="tag p0">P0</span> 违反红线 / <span class="tag p1">P1</span> 违反基线 / <span class="tag p2">P2</span> 可打磨。修法列标注对应主题页与线框徽标。</p>
  <table>
    {thead}
    {crows_b}
  </table>
