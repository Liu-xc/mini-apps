...
## ADR-005 · 掩码后处理加边缘锐化窗（2026-09-20，it-016 阶段 A 测试报告驱动）

- **决策**：min-max 归一化后增加锐化窗——掩码 <0.35 判背景（alpha 0）、>0.85 判主体（alpha 255）、之间线性过渡（`MaskToAlpha.EDGE_LO/EDGE_HI`）。
- **动机**：软边像素 RGB 仍是背景色，纯软边输出产生「背景色晕边」（测试报告挂拍场景有一圈明显木色描边）；锐化窗收窄半透明带后晕边基本消除，保留 1px 级抗锯齿。
- **代价（如实）**：掩码中等的弱显著区域被削切——纯白零纹理印花会被挖洞；人穿衣服场景的头部/手臂残缺。前者影响面小（真实印花有纹理），后者强化「人穿衣服建议放弃抠图」的产品结论。
- **演进**：真机样张不满意再调窗参；alpha matting（发丝级）为远期候选。
