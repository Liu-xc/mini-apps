#!/bin/bash
# M0 spike（GLM/智谱）：真调 chat/completions，锁定 baseUrl 路径风格、SSE 流式格式、tool_calls 契约
# 用法：GLM_API_KEY=你的Key ./tools/spike-glm.sh
#       也可 GLM_API_KEY=$(security find-generic-password -s com.spartapps.island -a api-key -w) ./tools/spike-glm.sh
# 可选：GLM_MODEL=glm-4.6 跳过模型探测；NO_PROXY=1 走直连
...
echo "== 0. 探测可用模型 =="
MODELS=(${GLM_MODEL:-glm-4.6 glm-4.5-air glm-4-flash glm-5 glm-5.3})
MODEL=""
for m in "${MODELS[@]}"; do
  echo "-- 尝试 $m"
...
[ -n "$MODEL" ] || { echo "所有候选模型都不可用，请到控制台核对模型名后 GLM_MODEL=xxx 重试"; exit 1; }

echo
echo "== 1. SSE 流式（前 30 行原始 event-stream）=="
$CURL -N "$BASE/chat/completions" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "{\"model\":\"$MODEL\",\"stream\":true,\"max_tokens\":32,\"messages\":[{\"role\":\"user\",\"content\":\"数到3\"}]}" \
  | head -30 || true   # head 截管会让 curl 报 56，pipefail 下不放行会中断脚本
...
# arguments 必须作为 JSON 字符串回喂（M0 实证：裸嵌对象 GLM 报 1210 参数有误）
