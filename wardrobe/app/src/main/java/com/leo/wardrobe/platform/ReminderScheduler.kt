...
import com.leo.wardrobe.domain.usecase.ItemWearStat
import com.leo.wardrobe.domain.usecase.StaleItemSelector
 ...
 * 候选规则在 domain/usecase/StaleItemSelector（it-021 抽出，JVM 可测）。
 */
...
class MemoryReminderWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val stats = StaleItemSelector.aggregateWearStats(data.items, data.outfits, data.wearLogs)
        val names = data.items.associate { it.id to it.name }
        val personNames = data.persons.associate { it.id to it.name }
        val candidates = StaleItemSelector.candidates(stats, names, prefs.days, now)
    ...
    private suspend fun notifyPick(
        ...
        val days = ((now - pick.lastWornAt) / StaleItemSelector.DAY_MS).coerceAtLeast(0)
