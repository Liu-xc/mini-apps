...
| `inkFaint` 次级 | `#747F75` | `#94A294` | 序号/说明/占位；浅色白底对比 4.17:1，与常见淡绿容器 ≥3.5:1 |
| `accent` 图形强调 | `#429E68` 青杉绿 | `#74C790` | 插画、装饰、非文字图形强调 |
| `accentContent` 强调文字/动作 | `#1D6845` | `#74C790` | 浅色小字号文字、链接及实心按钮底；深色沿用 accent |
| 主强调实心按钮文字 | `#FFFFFF` | `#10150F` | `onPrimary` / `onSecondary`，浅色深绿底白字对比 6.73:1 |
| `hairline` 分隔 | `#E3EBE0` | `#273127` | 细分隔线 |

标签 chip：`paper` 底 + `ink` 字（浅）；深色反之。破坏性操作用系统 error 色。

强调色按用途拆分，禁止浅色 `accent=#429E68` 直接承载白色小字（仅 3.32:1）。浅色实心强调按钮使用 `primary=#1D6845` + 白字；强调文字使用 `accentContent=#1D6845`；图形装饰使用 `accent=#429E68`。正文最低 4.5:1，次级文字最低 3:1，实心按钮文字最低 4.5:1（DESIGN.md §2.2）。
...
- **主 CTA 实心化（C5）**：页面唯一主动作（W5「去背景 · 一键透明底」）一律实心填充 `primary(#1D6845) + 白字`（6.7:1），禁止全宽描边幽灵态承载主操作；描边形制保留给次要/入口动作（如 W11「开始对话」）。
- **关键指引不降级（C1）**：空态引导、颜色/材质描述、工具结果正文等「用户必须读」的文本用 `ink`（14.6:1）；`inkFaint`（3.9:1，达标 ≥3:1）只服务次要说明与元信息（时间戳、计数口径、占位）。
- **状态条容器统一（C7）**：W5 补抠状态条（候选/已抠）与 W12 错误条同用「圆角容器 + hairline 边」形制；**棋盘格 = 透明底** 语义以状态条下 `bodySmall` 注记明示（候选态专属）。
