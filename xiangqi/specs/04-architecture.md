...
| JeVEvaluator（it-002） | `server.mjs` 内 | `.env.local` 加载 key → 串行队列对每手 `fenAfter` 调 `POST api.typesafe.ai/v1/systemone`（Choice: red_win/draw/black_win）→ 回填 `Move.eval` + 广播 `eval`；**按局可选**（开局参数 + `eval-toggle` 运行中切换）；**prompt 全英文**（state/instructions/criteria，历史走 ICCS）；`JEV_DISABLED` 关停；Node `https.request` 直连（本机 LibreSSL 被掐但 OpenSSL 正常） |
